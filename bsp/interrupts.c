...
static void irqEnable() {
  ...
  __asm volatile("bic		r0,r0,#0x80"); // Clear bit 8, (0x80) -- Causes
                                               // IRQs to be enabled.
  __asm volatile(
...
static void irqDisable() {
  __asm volatile("mrs		r0,cpsr");     // Read in the cpsr register.
  __asm volatile("orr		r0,r0,#0x80"); // Set bit 8, (0x80) -- Causes
                                               // IRQs to be disabled.
  __asm volatile(
      "msr		cpsr_c, r0"); // Write it back to the CPSR register.
}
...
void OS_CPU_IRQ_ISR_Handler() {
  /**
   *	WARNING:
   *		Compiler optimisations will likely prevent this code from
   *working!
   *
   *	This will be fixed later!
   **/
  //uart_string("in OS_CPU_IRQ_ISR_Handler");
  ...
  if (ulMaskedStatus &
      ~0xFFFFF300) {     // Note how we mask out the GPU interrupt Aliases.
  ...
  if (tmp & 0x100) {
    ...
    // ulMaskedStatus &= ~((1 << 7) | (1 << 9) | (1 << 10) | (1 << 18) | (1 <<
    // 19));
    if (ulMaskedStatus) {
      goto emit_interrupt;
    }
  ...
  if (tmp & 0x200) {
    ...
    irqNumber = 32 + 31;
    // Don't clear the interrupts in the basic pending, simply allow them to
    // processed here!
    if (ulMaskedStatus) {
      goto emit_interrupt;
    }
...
emit_interrupt:
  ...
  //__asm volatile("clz	r7,r5");				// r5 is the
  // ulMaskedStatus register. Leaving result in r6!
  //__asm volatile("sub r6,r7");

  if (g_VectorTable[irqNumber - lz].pfnHandler) {
    g_VectorTable[irqNumber - lz].pfnHandler(irqNumber,
                                             g_VectorTable[irqNumber].pParam);
  }
}
